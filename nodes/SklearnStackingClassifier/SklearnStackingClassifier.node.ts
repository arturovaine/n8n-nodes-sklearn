...
export class SklearnStackingClassifier implements INodeType {
  description: INodeTypeDescription = {
    displayName: 'Sklearn Stacking Classifier',
    name: 'sklearnStackingClassifier',
    icon: 'file:sklearn.svg',
    group: ['transform'],
    version: 1,
    subtitle: '={{$parameter["operation"]}}',
    description: 'Stacked generalization ensemble classifier',
    defaults: {
      name: 'Sklearn Stacking Classifier',
    },
    inputs: ['main'],
    outputs: ['main'],
    properties: [
        ...
        options: [
          {
            name: 'Train',
            value: 'train',
            description: 'Train stacking classifier',
            action: 'Train model',
          },
          {
            name: 'Predict',
      ...
      },
      {
        ...
        default: ['RandomForestClassifier', 'GradientBoostingClassifier'],
        description: 'Select base estimators for stacking',
      },
      {
        displayName: 'Final Estimator',
        name: 'finalEstimator',
        type: 'options',
        displayOptions: {
          show: {
        ...
        description: 'Meta-learner that combines base estimator predictions',
      },
      {
        ...
        default: 5,
        description: 'Number of cross-validation folds for generating meta-features',
      },
      {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    try {
      if (operation === 'train') {
        ...
        const finalEstimator = this.getNodeParameter('finalEstimator', 0) as string;
        ...
        const allEstimators = [...new Set([...estimators, finalEstimator])];
        const imports = allEstimators.map((e) => estimatorImports[e]).join('\n');
        const estimatorList = estimators.map((e, i) => `('${e.toLowerCase()}_${i}', ${e}())`).join(', ');
...
result = {
    ...
    'base_estimators': ${JSON.stringify(estimators)},
    'final_estimator': '${finalEstimator}',
...
`;
        ...
        returnData.push({
          json: {
            model: resultData,
            score: result.score,
            base_estimators: result.base_estimators,
